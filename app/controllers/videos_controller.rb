...
#rdoc comment is needed
class VideosController < ApplicationController

before_action :authenticate_user!, except: [:index, :home, :show, :destroy]
  #these spaces on empty lines should be removed, there is a setting in rubymine that will do this
  def new
  end
...
## these custom  actions need rdoc
  def get_video(title)
  ...
  def local_search(params)
    search_fields = []
     params.each do |key, value|
       # indenting here need fixing, should be two spaces not three
        if value == "true"
          search_fields << key
        end
     end
    # don't need to assign variable if it's not being used, ruby will return the value of the last line by default
